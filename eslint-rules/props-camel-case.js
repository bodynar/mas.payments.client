...
/**
 * ESLint rule: props-camel-case
 *
 * Требует, чтобы все пропсы компонентов и поля типов были названы в camelCase,
 * за исключением:
...
export default {
    meta: {
        type: "problem",
        docs: {
            description: "Enforce camelCase naming for component props and type fields",
        ...
        schema: [],
        messages: {
            propsCamelCase: "Property '{{name}}' should be named in camelCase.",
        },
    },
    ...
    create(context) {
        ...
        function isCamelCase(name) {
            // Разрешаем числа и буквы, начинающиеся с маленькой буквы
        ...
        // Функция для проверки, является ли свойство частью объектного литерала с API-типом
        ...
        // Функция для проверки, является ли свойство частью ApiModel или ApiResponse типа
        ...
        function isTypeProperty(node) {
            return node.parent?.type === 'TSPropertySignature' ||
                node.parent?.type === 'TSMethodSignature';
        }

        function isInterfaceOrTypeProperty(node) {
            let parent = node.parent;
            while (parent) {
        ...
        function isApiObjectProperty(node) {
            ...
            if (node.parent?.type === 'MemberExpression' && node.parent.property === node) {
                // Ищем переменную, к которой обращаемся
                const objectName = node.parent.object?.name;
                if (!objectName) return false;

                // Ищем объявление этой переменной в области видимости
                const scope = sourceCode.getScope(node);
                let currentScope = scope;

                while (currentScope) {
                    const variable = currentScope.set.get(objectName);
                    if (variable) {
                        // Проверяем определения переменной
                ...
                // Дополнительная проверка: ищем в тексте функции вызовы с дженериками
                const functionNode = node.parent;
                let current = functionNode;
                while (current && current.type !== 'FunctionDeclaration' && current.type !== 'ArrowFunctionExpression' && current.type !== 'FunctionExpression') {
        ...
        function checkPropertyName(node) {
            ...
            // Проверяем только если есть нарушение соглашения
            if (!isCamelCase(name) && hasSnakeCase(name)) {
                context.report({
                    node,
                    messageId: "propsCamelCase",
                    data: { name }
                });
            }
        }
        ...
        return {
            // JSX атрибуты (пропсы компонентов)
            JSXAttribute(node) {
            ...
            // Свойства TypeScript интерфейсов и типов
            ...
            // Свойства в объектных литералах (только если они часть типа)
            Property(node) {
                if (node.key?.type === 'Identifier' && !node.computed && isInterfaceOrTypeProperty(node)) {
                    checkPropertyName(node.key);
                }
            },

            // Обращения к свойствам объектов (например, response.expiration_date)
            MemberExpression(node) {
                if (node.property?.type === 'Identifier' && !node.computed) {
                    checkPropertyName(node.property);
                }
            }
        };
    }
